...
def generate_namespaced_slam_params(base_params_path, namespace):
    """
    Generate a temporary, namespaced SLAM Toolbox parameters file from a base file.

    This function reads a base SLAM Toolbox parameters file, recursively prepends a
    namespace to relevant frame IDs and topic names, and saves the result
    to a temporary file. This is critical for multi-robot SLAM, as each
    robot's SLAM stack must use its own namespaced resources.
    The 'map' frame is explicitly excluded from namespacing.

    Args:
        base_params_path (str): The file path to the base SLAM parameters YAML.
        namespace (str): The robot-specific namespace to apply (e.g., 'tb1').
    ...
    # The top-level key is 'slam_toolbox'. We need to namespace this key
    # and then apply internal namespacing to its 'ros__parameters' values.
    if 'slam_toolbox' not in params:
        raise ValueError(f"Invalid SLAM parameters file structure: {base_params_path}. Expected top-level key 'slam_toolbox'.")

    # Create the initial namespaced structure for the node
    namespaced_output_params = {
        f"/{namespace}/slam_toolbox": params['slam_toolbox']
    }

    # Define keys that represent frame IDs and topic names within slam_toolbox parameters
    # These are the keys that will be checked inside the 'ros__parameters' dictionary.
    frame_keys = [
        'odom_frame', 'base_frame'
        # 'global_frame' and 'map_frame' are intentionally 'map' and should not be namespaced
    ]
    topic_keys = [
        'scan_topic'
    ]
    ...
    # Recursively traverse the parameter dictionary to apply namespaces.
    # This function will modify the 'namespaced_output_params' dictionary in place.
    def patch_recursively(data, ns):
        if isinstance(data, dict):
            for key, value in data.items():
                if isinstance(value, str) and value:  # Process only non-empty strings.
                    ...
                    elif is_topic and not value.startswith('/'): # Check for absolute paths
                        data[key] = f"{ns}/{value}"
                patch_recursively(value, ns) # Continue traversal into nested structures.
        elif isinstance(data, list):  # Also traverse lists.
            for item in data:
                patch_recursively(item, ns)
    
    # Apply patching to the entire namespaced_output_params structure.
    # The recursive function will find the 'ros__parameters' sub-dictionary
    # and then apply namespacing to its contents.
    patch_recursively(namespaced_output_params, namespace)

    output_path = os.path.join(tempfile.gettempdir(), f"{namespace}_slam_params.yaml")
    with open(output_path, 'w') as f:
        yaml.dump(namespaced_output_params, f, default_flow_style=False)
